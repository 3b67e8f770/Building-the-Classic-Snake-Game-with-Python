...
def snake_coordinates(direction,snake,mouse,score):   #new snake
    _x=snake[-1][0]
    _y=snake[-1][1]
    if direction == "up":
        new_haed=[_x,_y+20]
...
##    time.sleep(0.01)
    # out of the screen?
    if snake[-1][0] not in range(-x_max,x_max) or snake[-1][1] not in range(-y_max,y_max):
        ups=False
        break
    # eat itself?
    for i in range(0,len(snake)-2):
        if snake[-1] == snake[i]:
            ups=False
            turtle.bye()
    snake,score,mouse=snake_coordinates(direction,snake,mouse,score)
